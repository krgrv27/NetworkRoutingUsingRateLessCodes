...
# define nmessgs 10                 // total number of messages
    ...
    while (messgID < nmessgs)
    {
        ...
        while (fwd != destination)        // until the destination has the message
        {
            ...
            for (packID = 0; packID < npacks; ++packID)  // initial transmissions of N packets
            {
                ...
                {
                    int f = nxtfwdset[i];
                    std::tr1::binomial_distribution<> roll(1, pe[fwd][f]);
                    int r = roll(eng); //sto.Binomial(1, pe[fwd][f]);   // sampling binomial random variable
                    if (!r)
                    {
            ...
            while (!fwd_select)			// if no forwarder has been selected yet
            {
                ntrans = ntrans + 1;
                int sum = 0;
                std::tr1::binomial_distribution<> roll1(1, 0.5);
                 
                while (sum == 0)
                {
                    for (int i = 0; i < npacks; ++i)
                    {
                        vect[i] = roll1(eng); //sto.Binomial(1, 0.5);
                        sum = sum + vect[i];
                    }
                }
                ...
                for (int i = 0; i < nfwds; ++i)		// send to all the potential forwarders
                {
                    ...
                    std::tr1::binomial_distribution<> roll2(1, pe[fwd][f]);
                    int r = roll2(eng); //sto.Binomial(1, pe[fwd][f]); 
                    
                    //int r = 1; //sto.Binomial(1, pe[fwd][f]);  // sample the binomial random variable
                    if (!r)
                    {
